    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Tabs value={tab} onValueChange={(v) => navigate(`/agents/${v}`)}>
          <PageTabBar
            items={[
              { value: "all", label: t("agent.tabs.all") },
              { value: "active", label: t("agent.tabs.active") },
              { value: "paused", label: t("agent.tabs.paused") },
              { value: "error", label: t("agent.tabs.error") },
            ]}
            value={tab}
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{t("agent.count", { count: filtered.length })}</p>
      )}
            ...
            return (
              <EntityRow
                ...
                leading={hasInvalidOrgChain ? (
                  <AlertTriangle className="h-3.5 w-3.5 text-amber-500" aria-label={t("agent.invalidReportingChain")} />
                ) : (
...
function OrgTreeNode({
  ...
  return (
    <div style={{ paddingLeft: depth * 24 }}>
      ...
      >
        {hasInvalidOrgChain ? (
          <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-amber-500" aria-label={t("agent.invalidReportingChain")} />
        ) : (
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {t("agent.live")}{liveCount > 1 ? ` (${liveCount})` : ""}
      </span>
    </Link>
  );
}
