...
export function FailedRunInboxRow({
  ...
  const rawError = runFailureMessage(run);
  const displayError = rawError === "Run exited with an error." ? t("inbox.runExitedWithError") : rawError;
...
function JoinRequestInboxRow({
  ...
  const label = (() => {
    ...
    const requesterName = nonEmptyLabel(joinRequest.requesterUser?.name);
    const requesterEmail = nonEmptyLabel(joinRequest.requesterUser?.email) ?? nonEmptyLabel(joinRequest.requestEmailSnapshot);
    const requesterId = nonEmptyLabel(joinRequest.requestingUserId);
    if (requesterName && requesterEmail) return `${requesterName} (${requesterEmail})`;
    if (requesterEmail) return requesterEmail;
    if (requesterName) return requesterName;
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="issues_i_touched">{t("inbox.myRecentTasks")}</SelectItem>
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {t("inbox.subTasksCount", { count: childCount })}
                        </span>
                      ) : undefined}
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("inbox.newTaskInGroup", { group: group.label })}
                              aria-label={t("inbox.newTaskInGroup", { group: group.label })}
                              onClick={(event) => {
                                event.stopPropagation();
