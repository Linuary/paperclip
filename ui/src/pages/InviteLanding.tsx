    ...
    <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        <div className="flex items-center gap-3">
          ...
          />
          <h1 className="text-lg font-semibold">{t('inviteLanding.requestJoin', { company: companyDisplayName })}</h1>
        </div>
        <div className="mt-4 space-y-3">
          ...
          <div className="border border-zinc-800 p-3">
            ...
            >
              {t('inviteLanding.companySettingsMembers')}
            </a>
          </div>
          <p className="text-sm text-zinc-400">
            {t('inviteLanding.askToVisitPrefix')}{' '}
            <a href={approvalUrl} className="text-zinc-200 underline underline-offset-2 hover:text-zinc-100">
              {t('inviteLanding.companySettingsMembers')}
            </a>{' '}
            {t('inviteLanding.askToVisitSuffix')}
          </p>
          <p className="text-xs text-zinc-500">
            {t('inviteLanding.refreshAfterApproved')}
          </p>
        </div>
        ...
        {onboardingTextUrl ? (
          <div className="mt-4 text-xs text-zinc-400">
            {t('inviteLanding.onboarding')} <span className="font-mono break-all">{onboardingTextUrl}</span>
          </div>
        ) : null}
      </div>
    </div>
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || t('inviteLanding.defaultCompany');
  ...
  const acceptMutation = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t('inviteLanding.failedToAccept'));
    },
  });
  ...
  const joinButtonLabel = useMemo(() => {
    if (!invite) return t('inviteLanding.continue');
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t('inviteLanding.inviteNotAvailableDesc')}
          </p>
        </div>
      </div>
    );
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ? t('inviteLanding.reviewAgentDetails')
                    : requiresHumanAccount
                      ? t('inviteLanding.createAccountFirst')
                      : t('inviteLanding.accountReady')}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">{t('inviteLanding.company')}</div>
                <div className="mt-1 text-sm text-zinc-100">{companyDisplayName}</div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">{t('inviteLanding.invitedBy')}</div>
            ...
            {showsAgentForm ? (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {t('inviteLanding.submitAgentDetailsDesc', { company: companyDisplayName })}
                  </p>
                </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? t('inviteLanding.createAccountDesc', { company: companyDisplayName })
                      : t('inviteLanding.signInDesc')}
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {t('inviteLanding.iAlreadyHaveAccount')}
                  </button>
                </div>
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({ tone: "error", message: t('inviteLanding.pleaseFillRequired') });
                      return;
                    }
                    authMutation.mutate();
                  }}
                ...
                <div>
                  <h2 className="text-lg font-semibold">
                    {isCurrentMember
                      ...
                      ? t('inviteLanding.completingCompanyAccess')
                      : invite.inviteType === "bootstrap_ceo"
