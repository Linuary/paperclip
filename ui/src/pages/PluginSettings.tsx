...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t('plugin.settings.instanceSettings'), href: "/company/settings/instance/general" },
      { label: t('plugin.settings.plugins'), href: "/company/settings/instance/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t('plugin.settings.pluginDetails') },
    ]);
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t('plugin.manager.noDescription');
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t('plugin.about.title')}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t('plugin.about.description')}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t('plugin.about.author')}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t('plugin.about.categories')}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t('plugin.about.none')}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t('plugin.envDriver.configureFromEnvironments')}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t('plugin.envDriver.envDriverDesc', { driverLabel: driverLabel || t('plugin.envDriver.thisPlugin') })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/instance/environments">
                      <Button variant="outline" size="sm">{t('plugin.envDriver.openEnvironments')}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
                <p className="text-sm text-muted-foreground">
                  {t('plugin.envDriver.noSettings')}
                </p>
              ) : null}
            </section>
          </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t('plugin.dashboard.title')}
                  </CardTitle>
                  <CardDescription>
                    {t('plugin.dashboard.description')}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t('plugin.dashboard.workerProcess')}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t('plugin.dashboard.status')}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t('plugin.dashboard.pid')}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t('plugin.dashboard.uptime')}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t('plugin.dashboard.pendingRpcs')}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t('plugin.dashboard.crashes')}
                                  </span>
                                  <span className="text-xs">
                                    {t('plugin.dashboard.consecutiveTotal', { consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t('plugin.dashboard.lastCrash')}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t('plugin.dashboard.noWorker')}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t('plugin.dashboard.recentJobRuns')}
                        </h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t('plugin.dashboard.noJobRuns')}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t('plugin.dashboard.recentWebhooks')}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t('plugin.dashboard.noWebhooks')}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t('plugin.dashboard.lastChecked', { time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t('plugin.dashboard.unavailable')}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t('plugin.logs.lastEntries', { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t('plugin.health.title')}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t('plugin.health.checking')}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t('plugin.health.overall')}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t('plugin.health.lifecycle')}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t('plugin.health.checksRunWhenReady')}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t('plugin.permissions.noPermissions')}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t('plugin.localFolders.selectCompany')}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t('plugin.localFolders.title')}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t('plugin.localFolders.failedToLoad')}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t('plugin.localFolders.loading')}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t('plugin.localFolders.savedOk')
          : t('plugin.localFolders.savedWithWarning'),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t('plugin.localFolders.saveFailed') });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t('plugin.localFolders.pathRequired') });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t('plugin.localFolders.mustBeAbsolutePath') });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t('plugin.localFolders.healthy') : t('plugin.localFolders.needsAttention')}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t('plugin.localFolders.readWrite') : t('plugin.localFolders.readOnly')}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t('plugin.localFolders.configured')} value={status?.configured ? t('plugin.localFolders.yes') : t('plugin.localFolders.no')} ok={!!status?.configured} />
        <FolderStatusMetric label={t('plugin.localFolders.readable')} value={status?.readable ? t('plugin.localFolders.yes') : t('plugin.localFolders.no')} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t('plugin.localFolders.writable')}
          value={access === "read" ? t('plugin.localFolders.notRequested') : status?.writable ? t('plugin.localFolders.yes') : t('plugin.localFolders.no')}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t('plugin.localFolders.configuredPath')}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t('plugin.localFolders.localFolderPath')}
        </label>
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={t('plugin.localFolders.pathPlaceholder')}
          />
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t('plugin.localFolders.validationProblems')}</div>
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={t('plugin.localFolders.requiredDirs')}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={t('plugin.localFolders.missingDirs')}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={t('plugin.localFolders.requiredFiles')}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={t('plugin.localFolders.missingFiles')}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-[10px]">
            {t('plugin.localFolders.notInspected')}
          </Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-[10px]">
            {t('plugin.localFolders.missingCount', { count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-[10px]">{t('plugin.localFolders.present')}</Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t('plugin.localFolders.noneDeclared')}</p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t('plugin.localFolders.rootNotInspected')}</p>
      ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t('plugin.configForm.saved') });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t('plugin.configForm.saveFailed') });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t('plugin.configForm.testPassed') });
      } else {
        setTestResult({ type: "error", text: result.message || t('plugin.configForm.testFailed') });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t('plugin.configForm.testFailed') });
    },
  });
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t('plugin.configForm.saving')}
            </>
          ) : (
            t('plugin.configForm.saveConfig')
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t('plugin.configForm.testing')}
              </>
            ) : (
              t('plugin.configForm.testConfig')
            )}
          </Button>
        )}
      </div>
