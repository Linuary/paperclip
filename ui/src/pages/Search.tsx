...
const SUBGROUP_I18N_KEYS: Record<SubGroupKey, string> = {
  issues: "search.scope.tasks",
...
const SCOPE_I18N_KEYS: Record<CompanySearchScope, string> = {
...
function describeScope(scope: CompanySearchScope, t: TFunction) {
  ...
  return t(SCOPE_I18N_KEYS[scope]);
}
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {t(SCOPE_I18N_KEYS[value as CompanySearchScope])}
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            ...
            {t("search.identifierLookupDescBefore")}{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">PAP-123</code>{" "}
            {t("search.identifierLookupDescAfter")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.quotedPhrases")}</span>{" "}
            {t("search.quotedPhrasesDesc")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.keyboardShortcut")}</span>{" "}
            {t("search.keyboardShortcutDesc")}
          </li>
        </ul>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? t("search.errorServerStatus", { status }) : t("search.errorRequestFailed")}{" "}
          {t("search.errorDesc")}
        </p>
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.noResultsDesc", { scope: describeScope(scope, t).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("search.tryFewerTokens")}</li>
          <li>
            {t("search.tryIdentifierBefore")} <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>{t("search.tryIdentifierAfter")}
          </li>
          <li>{t("search.wrapPhrases")}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {t("search.resultCount", { count: totalResults })} · {t("search.sortedByRelevance")}
        </span>
      ...
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={t(SUBGROUP_I18N_KEYS[group.key])}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={t(SUBGROUP_I18N_KEYS[group.key])}
                trailing={
