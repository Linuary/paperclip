...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.heartbeats.failedToUpdate"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.heartbeats.failedToDisableOne", { detail })
            : t("instanceSettings.heartbeats.failedToDisableMultiple", { failures: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.heartbeats.failedToDisableAll"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.heartbeats.failedToLoad")}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        {anyEnabled && (
          <Button
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("skills.agent") : t("skills.agents");
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.heartbeats.noHeartbeats")}
        />
      ) : (
