...
export function JoinRequestQueue() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("joinRequests.breadcrumbs.inbox"), href: "/inbox" },
      { label: t("joinRequests.breadcrumbs.joinRequests") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("joinRequests.requestApproved"), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("joinRequests.requestRejected"), tone: "success" });
    },
  });
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card p-4">
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="pending_approval">{t("joinRequests.pendingApproval")}</option>
            <option value="approved">{t("joinRequests.approved")}</option>
            <option value="rejected">{t("joinRequests.rejected")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="all">{t("joinRequests.all")}</option>
            <option value="human">{t("joinRequests.human")}</option>
            <option value="agent">{t("joinRequests.agent")}</option>
          </select>
        </label>
      </div>
                  ...
                  <div>
                    <div className="text-base font-medium">
                      {request.requestType === "human"
                        ? request.requesterUser?.name || request.requestEmailSnapshot || request.requestingUserId || t("joinRequests.unknownHumanRequester")
                        : request.agentName || t("joinRequests.unknownAgentRequester")}
                    </div>
