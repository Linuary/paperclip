...
export function ApprovalDetail() {
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("approval.reviewLinkedTasks")
              : t("approval.reviewLinkedTask"),
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("approval.decisionNote", { note: approval.decisionNote })}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("approval.linkedTasks")}</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("approval.linkedTasksDescription")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approval.resolveBudgetStop")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approval.confirmDeleteDisapprovedAgent"))) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approval.commentsCount", { count: comments?.length ?? 0 })}</h3>
        ...
        <Textarea
          ...
          placeholder={t("approval.addCommentPlaceholder")}
          rows={3}
        />
