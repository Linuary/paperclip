...
export function ProfileSettings() {
  ...
  const { currentLocale, supportedLanguages, changeLanguage } = useLanguage();
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.profile.title") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.profile.failedToUpdate"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("instanceSettings.profile.selectCompanyForAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.profile.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.profile.failedToRemoveAvatar"));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("instanceSettings.profile.failedToLoad")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("instanceSettings.profile.storedInCompany", { name: selectedCompany.name })
    : t("instanceSettings.profile.selectCompanyToUpload");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.profile.title")}</h1>
        </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {currentImage ? (
                    ...
                    >
                      ...
                      {t("instanceSettings.profile.remove")}
                    </Button>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("instanceSettings.profile.clickToUpload")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("instanceSettings.profile.email")}</Label>
            <Input
              id="profile-email"
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("instanceSettings.profile.saving") : t("instanceSettings.profile.saveProfile")}
            </Button>
          </div>
        </form>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-sm font-semibold">{t("settings.profile.language", { defaultValue: "Language" })}</h2>
          </div>
          <p className="text-sm text-muted-foreground mb-4">
            {t("settings.profile.languageDescription", { defaultValue: "Select your preferred display language." })}
          </p>
