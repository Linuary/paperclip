...
const concurrencyPolicyDescriptionKeys: Record<string, string> = {
  coalesce_if_active: "routine.policies.coalesceIfActive",
  always_enqueue: "routine.policies.alwaysEnqueue",
  skip_if_active: "routine.policies.skipIfActive",
};
const catchUpPolicyDescriptionKeys: Record<string, string> = {
  skip_missed: "routine.policies.skipMissed",
  enqueue_missed_with_cap: "routine.policies.enqueueMissedWithCap",
};
...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routine.toasts.created"),
        body: routine.assigneeAgentId
          ? t("routine.toasts.createdWithAgent")
          : t("routine.toasts.createdDraft"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routine.toasts.failedToUpdate"),
        body: mutationError instanceof Error ? mutationError.message : t("routine.toasts.couldNotUpdate"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routine.toasts.runFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routine.toasts.couldNotStartRun"),
        tone: "error",
      });
    },
  });
  ...
  const translateGroupLabel = (label: string | null): string => {
    if (label === null) return "";
    const map: Record<string, string> = {
      "No project": t("routine.groups.noProject"),
      "Unknown project": t("routine.groups.unknownProject"),
      "Unassigned": t("routine.groups.unassigned"),
      "Unknown agent": t("routine.groups.unknownAgent"),
    };
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routine.toasts.defaultAgentRequired"),
        body: t("routine.toasts.setDefaultAgent"),
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routine.selectCompany")} />;
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routine.title") },
            { value: "runs", label: t("routine.recentRuns") },
          ]}
        />
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t("routine.routineCount_other", { count: visibleRoutines.length })}
            </p>
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routine.sort.title")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routine.sort.title")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("routine.sort.updated")],
                      ["created", t("routine.sort.created")],
                      ["lastRun", t("routine.sort.lastRun")],
                      ["title", t("routine.sort.titleField")],
                      ...
                      >
                        ...
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc" ? t("routine.sort.asc") : t("routine.sort.desc")}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routine.group.title")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routine.group.title")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["project", t("routine.group.project")],
                      ["assignee", t("routine.group.agent")],
                      ["none", t("routine.group.none")],
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routine.composer.title")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routine.composer.description")}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routine.composer.routineTitle")}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("routine.composer.for")}</span>
                  <InlineEntitySelector
                    ref={assigneeSelectorRef}
                    value={draft.assigneeAgentId}
                    options={assigneeOptions}
                    recentOptionIds={recentAssigneeIds}
                    placeholder={t("routine.composer.assignee")}
                    noneLabel={t("routine.composer.noAssignee")}
                    searchPlaceholder={t("routine.composer.searchAssignees")}
                    emptyMessage={t("routine.composer.noAssigneesFound")}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("routine.composer.assignee")}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  />
                  <span>{t("routine.composer.in")}</span>
                  <InlineEntitySelector
                    ref={projectSelectorRef}
                    value={draft.projectId}
                    options={projectOptions}
                    recentOptionIds={recentProjectIds}
                    placeholder={t("routine.composer.project")}
                    noneLabel={t("routine.composer.noProject")}
                    searchPlaceholder={t("routine.composer.searchProjects")}
                    emptyMessage={t("routine.composer.noProjectsFound")}
                    onChange={(projectId) => {
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("routine.composer.project")}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={t("routine.composer.addInstructions")}
                bordered={false}
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routine.composer.advancedDelivery")}</p>
                    <p className="text-sm text-muted-foreground">{t("routine.composer.advancedDeliveryDescription")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routine.composer.concurrency")}</p>
                      <Select
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routine.composer.catchUp")}</p>
                      <Select
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routine.composer.footerDescription")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routine.composer.creating") : t("routine.createRoutine")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routine.composer.failedToCreate")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routine.failedToLoad")}
          </CardContent>
        </Card>
      ) : null}
        ...
        <div>
          {visibleRoutines.length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routine.noActiveRoutines")}
              />
            </div>
          ) : (
