          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {PRESET_KEYS.map((key) => (
                ...
                >
                  {t(`costs.dateRange.${key}`, { defaultValue: PRESET_LABELS[key] })}
                </Button>
              ))}
            </div>
          </div>
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.agentsPausedProjectsPaused", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.budgetOf", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.inOut", { input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs.apiSubscription", { api: row.apiRunCount, subscription: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
