...
export function IssueAssignedBacklogNotice({
  ...
  const assigneeLabel = assigneeAgent?.name ?? t('issue.assignedBacklog.defaultAssignee');
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <p className="leading-5">
            <span className="font-medium">{t('issue.assignedBacklog.parked')}</span> —{" "}
            <span className="font-medium">{assigneeLabel}</span> {t('issue.assignedBacklog.willNotBeWoken')}{" "}
          ...
          {assigneeAgent ? (
            <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
              {t('issue.assignedBacklog.commentsStillWake')}
            </p>
          ) : null}
          {onResume ? (
            <div className="pt-0.5">
              ...
              >
                {resuming ? t('issue.assignedBacklog.resuming') : t('issue.assignedBacklog.resumeNow')}
              </Button>
            </div>
          ) : null}
        </div>
