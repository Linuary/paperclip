        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t('billerSpendCard.in')}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t('billerSpendCard.out')}
              {" · "}
              {t('billerSpendCard.providerCount', { count: row.providerCount })}
              {" · "}
              {t('billerSpendCard.modelCount', { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {budgetMonthlyCents > 0 && (
          <QuotaBar
            ...
            rightLabel={t('billerSpendCard.ofAllocation', { percent: Math.round(budgetPct) })}
          />
        )}

        <div className="text-xs text-muted-foreground">
          {t('billerSpendCard.meteredRuns', { count: row.apiRunCount })}
          {" · "}
          {t('billerSpendCard.subscriptionRuns', { count: row.subscriptionRunCount })}
          {" · "}
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {t('billerSpendCard.tokenCount', { value: formatTokens(entry.inputTokens + entry.outputTokens) })}
                      </div>
                    </div>
                  </div>
                ))}
