    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="flex h-[calc(100dvh-2rem)] max-h-[calc(100dvh-2rem)] max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-[min(calc(100dvh-2rem),42rem)]">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pr-12 pt-6">
          ...
          <DialogTitle>{t("routine.dialog.runRoutine")}</DialogTitle>
          <DialogDescription>
            {t("routine.dialog.runRoutineDescription")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routine.dialog.agentRequired")}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routine.dialog.agentLabel")}
                noneLabel={t("routine.dialog.selectAgent")}
                searchPlaceholder={t("routine.dialog.searchAgents")}
                emptyMessage={t("routine.dialog.noAgentsFound")}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("routine.dialog.selectAgent")}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routine.dialog.projectLabel")}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("routine.dialog.projectLabel")}
                noneLabel={t("routine.dialog.noProject")}
                searchPlaceholder={t("routine.dialog.searchProjects")}
                emptyMessage={t("routine.dialog.noProjectsFound")}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("routine.dialog.noProject")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routine.dialog.noValue")}</SelectItem>
                    <SelectItem value="true">{t("routine.dialog.true")}</SelectItem>
                    <SelectItem value="false">{t("routine.dialog.false")}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("routine.dialog.chooseAValue")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routine.dialog.noValue")}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("routine.dialog.defaultAgentRequired")}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routine.dialog.missing", { fields: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routine.dialog.chooseWorkspaceBeforeRunning")}
            </p>
          ) : (
          ...
          >
            {isPending ? t("routine.dialog.running") : t("routine.dialog.runRoutine")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
