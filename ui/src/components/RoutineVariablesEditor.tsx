      ...
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {t('routineVariablesEditor.detectedFromPlaceholders', { name: '{{name}}' })}
          </p>
        </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="true">{t('routineVariablesEditor.trueValue')}</SelectItem>
                      <SelectItem value="false">{t('routineVariablesEditor.falseValue')}</SelectItem>
                    </SelectContent>
                  </Select>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t('routineVariablesEditor.defaultValue')}
                  />
                )}
              </div>
            </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t('routineVariablesEditor.usePlaceholders', { name: '{{variable_name}}' })}
        </span>
        <button
          type="button"
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('routineVariablesEditor.howToPrompt')}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              ...
              <p className="text-muted-foreground">
                {t('routineVariablesEditor.customVariablesDescription', { name: '{{variable_name}}' })}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t('routineVariablesEditor.namesMustStartWith')}</li>
                <li>{t('routineVariablesEditor.pickAType')}</li>
                <li>{t('routineVariablesEditor.sameNameReused')}</li>
              </ul>
            </section>
              ...
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t('routineVariablesEditor.placeholderHeader')}</th>
                      <th className="px-3 py-2 font-medium">{t('routineVariablesEditor.exampleHeader')}</th>
                  ...
                  <tbody className="divide-y divide-border/70">
                    {BUILTIN_VARIABLE_DOCS.map((entry) => (
                      <tr key={entry.name} className="align-top">
                        ...
                        <td className="px-3 py-2 text-muted-foreground">{entry.name === "date" ? t('routineVariablesEditor.builtinDateDescription') : t('routineVariablesEditor.builtinTimestampDescription')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
