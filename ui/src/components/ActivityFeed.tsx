    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-[#959596]">{t("activity.madeUpdatesTo", { count: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (allPaused) {
      return {
        text: t("activity.allAgentsPaused"),
        showPulse: false,
      };
    }
    return {
      text: t("activity.agentsRunning"),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("activity.agentFeed")}</h3>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("activity.groupByTaskAria")}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat" ? t("activity.groupByTask") : t("activity.showFlat")}
            </TooltipContent>
          </Tooltip>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("activity.filterByAria")}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("activity.filterBy")}</TooltipContent>
            </Tooltip>
              ...
              >
                {FILTER_OPTIONS.map(({ value }) => (
                  <DropdownMenuRadioItem key={value} value={value}>
                    {value === "all" ? t("activity.all") : value === "in-progress" ? t("activity.inProgress") : value === "for-review" ? t("activity.inReview") : t("activity.done")}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
              <DropdownMenuSeparator />
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-[16rem]">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("activity.defaultEmptyMessage")}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
