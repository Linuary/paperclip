...
function IssueAssigneePausedNotice({ agent }: { agent: Agent | null }) {
  ...
  const pauseDetail =
    agent.pauseReason === "budget"
      ? t("issue.chat.pausedByBudget")
      : agent.pauseReason === "system"
        ? t("issue.chat.pausedBySystem")
        : t("issue.chat.pausedManually");
    ...
    <div className="mb-3 rounded-md border border-orange-300/70 bg-orange-50/90 px-3 py-2.5 text-sm text-orange-950 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-100">
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          <span className="font-medium">{agent.name}</span> {t("issue.chat.pausedNotice")} {pauseDetail}
        </p>
      </div>
    </div>
  );
      ...
      <div className="rounded-xl border border-amber-300/60 bg-amber-50/80 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/20 dark:text-amber-200">
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            <p className="font-medium">{t("issue.chat.chatRendererError")}</p>
            <p className="text-xs opacity-80">
              {t("issue.chat.showingFallback")}
            </p>
          </div>
        </div>
      </div>
...
function IssueChatChainOfThought({
  ...
  if (isActive) {
    headerVerb = t("issue.chat.working");
    if (liveElapsed) headerSuffix = t("issue.chat.forDuration", { duration: liveElapsed });
  } else if (segmentTiming) {
    const durationMs = segmentTiming.endMs - segmentTiming.startMs;
    const durationText = formatDurationWords(durationMs);
    headerVerb = t("issue.chat.worked");
    if (durationText) headerSuffix = t("issue.chat.forDuration", { duration: durationText });
  } else {
    headerVerb = t("issue.chat.worked");
  }
        ...
        onClick={() => {
          void copyTextToClipboard(children).then(() => {
            ...
            toastActions?.pushToast({
              title: t("issue.chat.copyFailed"),
              body: error instanceof Error ? error.message : t("issue.chat.unableToCopyText"),
              tone: "error",
            });
          });
        }}
  ...
  const messageBody = (
    <div className={cn("flex min-w-0 max-w-[85%] flex-col", isCurrentUser && "items-end")}>
      ...
      >
        {queued ? (
          <div className="mb-1.5 flex items-center gap-2">
            ...
            {queueTargetRunId && onInterruptQueued ? (
              ...
              >
                {isInterruptingQueuedRun ? t("issue.chat.interrupting") : t("issue.chat.interrupt")}
              </Button>
            ) : null}
            {onCancelQueued ? (
              <Button
        ...
        {deleted ? (
          <div className="text-sm italic text-muted-foreground">{t("issue.chat.commentDeleted")}</div>
        ) : (
        ...
        >
          ...
          {!deleted ? (
            <button
              ...
              title={t("issue.chat.copyMessage")}
              aria-label={t("issue.chat.copyMessage")}
              onClick={() => {
                ...
                void copyTextToClipboard(text).then(() => {
                  ...
                  toastActions?.pushToast({
                    title: t("issue.chat.copyFailed"),
                    body: error instanceof Error ? error.message : t("issue.chat.unableToCopyMessage"),
                    tone: "error",
                  });
                });
              }}
          ...
          {canDeleteComment ? (
            <button
              ...
              title={t("issue.chat.deleteComment")}
              aria-label={t("issue.chat.deleteComment")}
  ...
  return (
    <>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("issue.chat.deleteCommentTitle")}</DialogTitle>
            <DialogDescription>
              {t("issue.chat.deleteCommentDescription")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            title={t("issue.chat.moreActions")}
            aria-label={t("issue.chat.moreActions")}
          >
        ...
        <DropdownMenuContent align="end">
          ...
          >
            ...
            {t("issue.chat.copyMessageMenuItem")}
          </DropdownMenuItem>
  ...
  return (
    <>
      <button
        ...
        title={t("issue.chat.helpful")}
        aria-label={t("issue.chat.helpful")}
      ...
      <Popover open={reasonOpen} onOpenChange={setReasonOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            title={t("issue.chat.needsWork")}
            aria-label={t("issue.chat.needsWork")}
        ...
        <PopoverContent side="top" align="start" className="w-80 p-3">
          <div className="mb-2 text-sm font-medium">{t("issue.chat.whatCouldBeBetter")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("issue.chat.addShortNote")}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {isSaving ? t("issue.chat.saving") : t("issue.chat.saveNote")}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("issue.chat.feedbackSharingTitle")}</DialogTitle>
            <DialogDescription>
              {t("issue.chat.feedbackSharingDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("issue.chat.voteAlwaysSaved")}</p>
            <p>
              {t("issue.chat.chooseAlwaysAllow")}
            </p>
            <p>{t("issue.chat.changeInSettings")}</p>
            {termsUrl ? (
              <a
                href={termsUrl}
                target="_blank"
          ...
          <DialogFooter>
            ...
            >
              {isSaving ? t("issue.chat.saving") : t("issue.chat.dontAllow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingSharingDialog || isSaving}
            ...
            >
              {isSaving ? t("issue.chat.saving") : t("issue.chat.alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  const rowContent = (
    <div className="min-w-0 flex-1">
      <div className={cn("flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs", isCurrentUser && "justify-end")}>
        ...
        <span className="text-muted-foreground">{t("issue.chat.updatedThisTask")}</span>
        <a
          href={anchorId ? `#${anchorId}` : undefined}
        ...
        >
          ...
          {expanded ? t("issue.chat.hideConfirmation") : t("issue.chat.expiredConfirmation")}
        </button>
      </div>
      {expanded ? (
...
function StaleDispositionWarningDetails({
  ...
  if (sections.length === 0) {
    return <div className="text-xs leading-5 text-muted-foreground">{t("issue.chat.noAdditionalDetails")}</div>;
  }
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            <span className="text-sm font-medium text-foreground/80">
              {t("issue.chat.staleDispositionWarning")}
            </span>
            <span className="ml-auto flex items-center gap-1.5">
              {message.createdAt ? (
...
function SystemNoticeCommentRow({
  ...
  const handleCopy = () => {
    void copyTextToClipboard(bodyText).then(() => {
      ...
      toastActions?.pushToast({
        title: t("issue.chat.copyFailed"),
        body: error instanceof Error ? error.message : t("issue.chat.copyFailedNotice"),
        tone: "error",
      });
    });
  };
  ...
  const handleCopyLink = () => {
    ...
    void copyTextToClipboard(url).then(() => {
      ...
      toastActions?.pushToast({
        title: t("issue.chat.copyFailed"),
        body: error instanceof Error ? error.message : t("issue.chat.copyFailedNoticeLink"),
        tone: "error",
      });
    });
  };
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 px-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {anchorId ? (
            <button
              type="button"
              className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
              title={t("issue.chat.copyLink")}
              aria-label={t("issue.chat.copyLink")}
          ...
          <button
            type="button"
            className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
            title={t("issue.chat.copyNoticeText")}
            aria-label={t("issue.chat.copyNoticeText")}
            onClick={handleCopy}
          >
            {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
          </button>
      ...
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">
            {custom.followUpRequested === true ? t("issue.chat.requestedFollowUp") : t("issue.chat.updatedThisTask")}
          </span>
          <a
            href={anchorId ? `#${anchorId}` : undefined}
  ...
  return (
    <div id={anchorId} className="flex items-start gap-2.5 py-1.5">
      ...
      <div className="min-w-0 rounded-md border border-dashed border-border bg-muted/20 px-3 py-2 text-sm text-muted-foreground">
        ...
        <span> {t("issue.chat.deletedThisComment")}</span>
  ...
  return (
    ...
    >
        ...
        >
          <div className="flex max-w-md items-center gap-3 rounded-md bg-background/80 px-3 py-2 text-left shadow-sm ring-1 ring-border/60">
            ...
            <div className="min-w-0">
              <div className="text-sm font-medium text-foreground">{t("issue.chat.dropToUpload")}</div>
              <div className="mt-0.5 text-xs leading-5 text-muted-foreground">
                {t("issue.chat.dropDescription")}
              </div>
            </div>
          </div>
        </div>
      ...
      <MarkdownEditor
        ...
        value={body}
        onChange={setBody}
        placeholder={t("issue.chat.reply")}
        mentions={mentions}
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            <Popover open={workModeMenuOpen} onOpenChange={setWorkModeMenuOpen}>
              ...
              >
                ...
                <div className="mt-1 border-t px-2 py-1.5 text-[10px] text-muted-foreground">
                  {t("issue.chat.cmdCtrlCycleModes")}
                </div>
              </PopoverContent>
            </Popover>
          ) : null}
        ...
        {enableReassign && reassignOptions.length > 0 ? (
          <InlineEntitySelector
            ...
            placeholder={t("issue.chat.assignee")}
            noneLabel={t("issue.chat.noAssignee")}
            searchPlaceholder={t("issue.chat.searchAssignees")}
            emptyMessage={t("issue.chat.noAssigneesFound")}
        ...
        <Button size="sm" disabled={!canSubmit} onClick={() => void handleSubmit()}>
          {submitting ? t("issue.chat.posting") : t("issue.chat.send")}
        </Button>
      </div>
      ...
      <AlertDialog open={noAssigneeDialogOpen} onOpenChange={setNoAssigneeDialogOpen}>
        ...
        >
          <AlertDialogHeader>
            <AlertDialogTitle>{t("issue.chat.noAssigneeSelected")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("issue.chat.noAssigneeDescription")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
...
export function IssueChatThread({
  ...
  const resolvedEmptyMessage = emptyMessage
    ?? (variant === "embedded"
      ? t("issue.chat.noRunOutputYet")
      : t("issue.chat.emptyConversation"));
              ...
              {showComposer ? (
                <div data-testid="issue-chat-thread-notices" className="space-y-2">
                  ...
                  {legacyRecoverySourceIssue ? (
                    <SystemNotice
                      tone="info"
                      label={t("issue.chat.legacyRecoveryTask")}
                      body={
                        <span>
                          {t("issue.chat.legacyRecoveryDescription")}
