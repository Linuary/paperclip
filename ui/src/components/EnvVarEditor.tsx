...
export function EnvVarEditor({
  ...
  async function sealRow(index: number) {
    ...
    const name = window.prompt(t("envVarEditor.secretName"), suggested)?.trim();
    if (!name) return;
          ...
          <div key={index} className="flex items-center gap-1.5">
            ...
            {row.source === "secret" ? (
              <>
                  ...
                  <SelectContent>
                    {row.secretId && !secrets.some((s) => s.id === row.secretId) ? (
                      <SelectItem value={row.secretId}>
                        {t("envVarEditor.missing")} ({row.secretId.slice(0, 8)}…)
                      </SelectItem>
                    ) : null}
                    {secrets.map((secret) => (
                ...
                >
                  <SelectTrigger className={cn(selectTriggerClass, "flex-[1]")} aria-label={t("envVarEditor.version")}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <button
                  ...
                  title={t("envVarEditor.createSecretFromPlain")}
                >
                  {t("envVarEditor.new")}
                </button>
              </>
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(index)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("envVarEditor.storeAsSecret")}
                >
                  {t("envVarEditor.seal")}
                </button>
              </>
      ...
      {(() => {
        ...
        return (
          <p className="text-[11px] text-amber-700 dark:text-amber-400 inline-flex items-start gap-1">
            ...
            <span>
              {t("envVarEditor.secretBindingsNeedAttention", { count: issues.length, plural: issues.length === 1 ? "" : "s" })}{" "}
