...
export function ExecutionParticipantPicker({
  ...
  const participantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) return userLabel(value.slice("user:".length)) ?? t("executionParticipant.user");
    return value;
  };
  ...
  const label = stageType === "review" ? t("executionParticipant.reviewers") : t("executionParticipant.approvers");
    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setSearch(""); }}>
      ...
      <PopoverContent className="p-1 w-56" align="start" collisionPadding={16}>
        <input
          ...
          placeholder={stageType === "review" ? t("executionParticipant.searchReviewers") : t("executionParticipant.searchApprovers")}
          value={search}
        ...
        <div className="max-h-48 overflow-y-auto overscroll-contain">
          ...
          >
            {stageType === "review" ? t("executionParticipant.noReviewers") : t("executionParticipant.noApprovers")}
          </button>
          {currentUserId && (
            <button
              className={cn(
            ...
            >
              ...
              {t("executionParticipant.assignToMe")}
            </button>
          )}
          {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
            ...
            >
              <User className="h-3 w-3 shrink-0 text-muted-foreground" />
              {creatorUserLabel ?? t("executionParticipant.requester")}
            </button>
          )}
          {otherUserOptions
            .filter((option) => {
