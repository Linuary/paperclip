    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budgetPolicyCard.percentOfLimit", { percent: summary.utilizationPercent }) : t("budgetPolicyCard.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicyCard.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("common.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.paused && summary.pauseReason
            ? t("budgetPolicyCard.softAlertAtWithPause", { percent: summary.warnPercent, reason: summary.pauseReason })
            : t("budgetPolicyCard.softAlertAt", { percent: summary.warnPercent })}
        </div>
      </div>
    </div>
  ) : (
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budgetPolicyCard.projectPausedMessage")
          : t("budgetPolicyCard.scopePausedMessage")}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("budgetPolicyCard.budgetLabel")}
        </label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving ? t("common.saving") : summary.amount > 0 ? t("budgetPolicyCard.updateBudget") : t("budgetPolicyCard.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("common.paused") : summary.status === "warning" ? t("budgetPolicyCard.warning") : summary.status === "hard_stop" ? t("budgetPolicyCard.hardStop") : t("budgetPolicyCard.healthy")}
          </div>
        </div>
