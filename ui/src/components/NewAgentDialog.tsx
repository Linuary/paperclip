...
export function NewAgentDialog() {
  ...
  function handleAskCeo() {
    ...
    openNewIssue({
      ...
      title: t("agent.dialog.askCeoTitle"),
      description: t("agent.dialog.askCeoDescription"),
    });
  }
  ...
  async function copyText(text: string, unavailableBody: string) {
    ...
    pushToast({
      title: t("agent.dialog.clipboardUnavailable"),
  ...
  const createAgentInviteMutation = useMutation({
    ...
    onSuccess: async (invite) => {
      ...
      setLatestAgentPromptCopied(false);
      setMode("prompt");
      const copied = await copyText(prompt, t("agent.dialog.copyManuallyBelow"));

      await queryClient.invalidateQueries({ queryKey: inviteHistoryQueryKey });
      pushToast({
        title: t("agent.dialog.inviteCreated"),
        body: copied ? t("agent.dialog.promptReadyCopied") : t("agent.dialog.promptReady"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("agent.dialog.inviteCreateFailed"),
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <span className="text-sm text-muted-foreground">{t("agent.dialog.addNewAgent")}</span>
          <Button
            variant="ghost"
            size="icon-xs"
          ...
          {mode === "choices" ? (
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <h2 className="text-sm font-semibold">{t("agent.dialog.inviteHeading")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("agent.dialog.inviteDescription")}
                  </p>
                </div>
              </div>

              <label className="block space-y-2">
                <span className="text-sm font-medium">{t("agent.dialog.optionalMessage")}</span>
                <Textarea
                  value={agentMessage}
                  onChange={(event) => setAgentMessage(event.target.value)}
                  className="min-h-24 resize-y"
                  placeholder={t("agent.dialog.optionalMessagePlaceholder")}
                  maxLength={4000}
                />
              </label>

              <div className="rounded-lg border border-border px-4 py-3 text-sm text-muted-foreground">
                {t("agent.dialog.inviteInfo")}
              </div>
              ...
              <div>
                ...
                >
                  {createAgentInviteMutation.isPending ? t("agent.dialog.generating") : t("agent.dialog.generatePrompt")}
                </Button>
              </div>
            </div>
          ) : (
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-3">
                    <h2 className="text-sm font-semibold">{t("agent.dialog.onboardingPrompt")}</h2>
                    {latestAgentPromptCopied ? (
                      <div className="inline-flex items-center gap-1 text-xs font-medium text-foreground">
                        <Check className="h-3.5 w-3.5" />
                        {t("agent.dialog.copied")}
                      </div>
                    ) : null}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {t("agent.dialog.sendPromptHint")}
                  </p>
                </div>
              </div>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const copied = await copyText(latestAgentPrompt, t("agent.dialog.copyManuallyAbove"));
                  setLatestAgentPromptCopied(copied);
                }}
              >
                {latestAgentPromptCopied ? t("agent.dialog.copiedPrompt") : t("agent.dialog.copyPrompt")}
              </Button>
            </div>
          )}
        </div>
