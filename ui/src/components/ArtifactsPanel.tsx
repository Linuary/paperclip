...
export function ArtifactsPanel({ taskId, isAgentWorking, openDocKey, openDocTitle, onClearOpenDoc, onApprove, onReject }: ArtifactsPanelProps) {
  ...
  const effectiveViewingDoc = openDocKey
    ? { key: openDocKey, title: openDocTitle ?? t("artifact.docFallbackTitle") }
    : viewingDoc;
      ...
      <div className="px-4 py-2 flex flex-wrap gap-1 border-b border-border">
        {FILTERS.map((f) => {
          const filterLabelMap: Record<string, string> = {
            all: t("artifact.filterAll"),
            in_progress: t("artifact.filterInProgress"),
            for_review: t("artifact.filterForReview"),
            completed: t("artifact.filterCompleted"),
          };
          return (
          <button
            key={f.value}
          ...
          >
            {filterLabelMap[f.value] ?? f.label}
          </button>
          );
        })}
      </div>
          ...
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {workProducts?.length === 0
                ? t("artifact.noDeliverablesYet")
                : t("artifact.noMatchFilter")}
            </p>
          </div>
        ) : (
                          ...
                          <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full", badge.className)}>
                            {(() => {
                              const statusLabelMap: Record<string, string> = {
                                "In Progress": t("artifact.statusInProgress"),
                                "For Review": t("artifact.statusForReview"),
                                "Completed": t("artifact.statusCompleted"),
                                "Changes Requested": t("artifact.statusChangesRequested"),
                              };
                              return statusLabelMap[badge.label] ?? badge.label;
                            })()}
                          </span>
                        )}
                      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            {t("artifact.docLoading")}
          </div>
        ) : error ? (
          <p className="text-sm text-muted-foreground">{t("artifact.docNotAvailable")}</p>
        ) : doc?.body ? (
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <MarkdownBody>{doc.body}</MarkdownBody>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">{t("artifact.docEmpty")}</p>
        )}
      </div>
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-[11px] text-muted-foreground mb-2">{t("artifact.needsReview")}</p>
        ...
        <div className="border-t border-orange-500/30 bg-orange-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[13px] font-medium text-orange-700 dark:text-orange-400">
              {t("artifact.changesRequestedRevising")}
            </p>
          </div>
        </div>
      )}
