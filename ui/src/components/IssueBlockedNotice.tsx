...
function BlockerRecoveryIndicator({ action }: { action: IssueRecoveryAction }) {
  ...
  return (
    <span
      ...
      role="status"
      aria-label={label}
      title={t('issue.row.recoveryTooltip', { label })}
...
function SuccessfulRunRetryNowControl({
  ...
  const scheduleLabel = relative === "now"
    ? t('issue.blocked.dueNow')
    : relative
      ? t('issue.blocked.scheduledRelative', { relative })
      : t('issue.blocked.scheduled');
    ...
    <div className="mt-2 rounded-md border border-amber-300/70 bg-background/80 p-2 dark:border-amber-500/40 dark:bg-background/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 text-xs leading-5 text-amber-900 dark:text-amber-100">
          {t('issue.blocked.correctiveWake', { scheduleLabel })}
        </div>
        <Button
          type="button"
          variant="outline"
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {t('issue.blocked.retrying')}
            </span>
          ) : success ? (
            <span className="inline-flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />
              {retryNow.data?.outcome === "already_promoted" ? t('issue.blocked.alreadyPromoted') : t('issue.blocked.promoted')}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5">
              <RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />
              {t('issue.blocked.retryNow')}
            </span>
          )}
        </Button>
      </div>
...
export function IssueBlockedNotice({
  ...
  const blockerLabel = blockers.length === 1 ? t('issue.blocked.linkedTask') : t('issue.blocked.linkedTasks');
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t('issue.blocked.needsNextStep')}</p>
              <p className="leading-5">
                {t('issue.blocked.runFinishedButOpen')}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t('issue.blocked.markDoneOrCancelled')}</li>
                <li>{t('issue.blocked.sendForReview')}</li>
                <li>{t('issue.blocked.markBlockedWithOwner')}</li>
                <li>{t('issue.blocked.delegateFollowUp')}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t('issue.blocked.correctiveWakeQueuedFor', { agent: agentName ?? t('issue.blocked.theAssignee') })}
                </span>
              </div>
              {successfulRunHandoff.detectedProgressSummary ? (
                <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
                  {t('issue.blocked.detectedProgress', { summary: successfulRunHandoff.detectedProgressSummary })}
                </p>
              ) : null}
                ...
                {blockers.length > 0
                  ? isStalled
                    ? stalledLeafBlockers.length > 1
                      ? t('issue.blocked.stalledMultiple', { blockerLabel })
                      : t('issue.blocked.stalledSingle', { blockerLabel })
                    : t('issue.blocked.blockedByLinked', { blockerLabel, pronoun: blockers.length === 1 ? t('issue.blocked.itIs') : t('issue.blocked.theyAre') })
                  : t('issue.blocked.blockedUntilTodo')}
              </p>
              {blockers.length > 0 ? (
              ...
              {showStalledRow ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t('issue.blocked.stalledInReview')}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t('issue.blocked.ultimatelyWaitingOn')}
                  </span>
              ...
              {showParkedRow ? (
                ...
                >
                  <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-800 dark:text-amber-200">
                    ...
                    {t('issue.blocked.blockedByParkedWork')}
                  </span>
