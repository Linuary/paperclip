...
export function WorkspaceRuntimeControls({
  ...
  const { t } = useTranslation();
  const serviceEmptyMessage = serviceEmptyMessageProp ?? t("workspaceRuntimeControls.noServicesConfigured");
  const jobEmptyMessage = jobEmptyMessageProp ?? t("workspaceRuntimeControls.noJobsConfigured");
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceRuntimeControls.title")}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? t("workspaceRuntimeControls.servicesRunning", { count: runningCount }) : t("workspaceRuntimeControls.noServicesRunning")}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? t("workspaceRuntimeControls.jobsAvailable", { count: resolvedSections.jobs.length })
                : t("workspaceRuntimeControls.eachCommand")}
            </span>
          </div>
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          ...
          description={t("workspaceRuntimeControls.untrackedDescription")}
