...
export function IssueOutputSection({ workProducts, resolveCreatorName }: IssueOutputSectionProps) {
  ...
  return (
    <section className="space-y-3" aria-label={t('issue.output.taskOutputs')}>
      <div className="flex items-center gap-2">
        <Play className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
        <h3 className="text-sm font-medium text-muted-foreground">{t('issue.output.output')}</h3>
      ...
      {rest.length > 0 && (
        <div className="space-y-2">
          <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t('issue.output.alsoProduced')}</p>
          {rest.map((item) => (
